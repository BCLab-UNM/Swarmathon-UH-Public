...
Result LogicController::DoWork() {
  ...
  //if any controller needs an interrupt the logic state is changed to interrupt
  for(PrioritizedController cntrlr : prioritizedControllers) {
    if(cntrlr.controller->ShouldInterrupt() && cntrlr.priority >= 0)
    /*shouldInterupt() stats:
    DriveController -     return false
    DropOffController -   processData(), returns true, precisionInterrupt = true if enough homecubes are seen
    ManualWaypointController -  false unless a new manualWaypoint has been added (only in manual mode for now)
    ObstacleController -  true if we have detected an obstacle and sets obstacleInterrupt to true
    PickUpController -    processData(), if release control then true, or if tagetHeld, or target founf && !interupted then true, or if !targetFound && interupted then true
    SearchController -    return false
    */
      {
      	logicState = LOGIC_STATE_INTERRUPT;
  ...
  //when an interrupt has been thrown or there are no pending control_queue.top().actions logic controller is in this state.
  case LOGIC_STATE_INTERRUPT: {
    ...
    for(PrioritizedController cntrlr : prioritizedControllers) {
      if(cntrlr.controller->HasWork()) {
        /*
        DropOffController -   true if isPrecisionDriving || startWaypoint
        SearchController -    true
        PickUpController -    true if target found
        ObstacleController -  return if a waypoint needs to be set or if obstacle hasnt been avoided
        */
    ...
    ////result = searchController.DoWork();//debug: only run search (no obstacle detection/pickupdropoff)
    ...
    if(result.type == behavior) {//drivecontroller, dropOffController, pickUpController
      ...
      if(result.b == nextProcess) {//pickup or dropoff controllers
  ...
  case LOGIC_STATE_PRECISION_COMMAND: {
    cout << "LOGIC_STATE_PRECISION_COMMAND" << endl;
