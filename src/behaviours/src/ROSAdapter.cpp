...
int broadcastName = 1; //sets if swarmie needs to broadcast its name
int swarmCount = 0; //number of swarmies in swarm

string publishedName;
stringstream ssnameList;
...
//Shares name and random number to differentiate swarmies
//ros::Publisher hierarchyPublisher;
...
ros::Subscriber nameListSubscriber;
...
unsigned int startDelayInSeconds = 20;
...
void nameListHandler(const std_msgs::String::ConstPtr& names);//list of all swarmie names connected
...
int main(int argc, char **argv) {
  ...
  ///////////Patrick's code
  nameListSubscriber = mNH.subscribe(("/nameList"), 10, nameListHandler);//gets a list of all swarmie names in format: 3 alpha bravo charlie

  //hierarchyPublisher = mNH.advertise<std_msgs::Float64>((publishedName +"/hierarchyPre"), 1, true);
  namePublisher = mNH.advertise<std_msgs::String>(("/nameListPre"), 1, true);//publishes host name to MASTER_NAMELIST node
  /////////////
...
void behaviourStateMachine(const ros::TimerEvent&)
{
  ...
  if (!initialized)
  {
    ...
    if (timerTimeElapsed > startDelayInSeconds)//can adjust start delay to make sure odometry handler has run atleast once
    //might be better than double initialize to for when this code runs at first
    {
      timeCnt = 0;
      // initialization has run
      initialized = true;//sets the home and map centers
      //TODO: this just sets center to 0 over and over and needs to change
      Point centerOdom;
      centerOdom.x = 1.3 * cos(currentLocation.theta);//sets home to 1.3 meters away
      centerOdom.y = 1.3 * sin(currentLocation.theta);
      //do we want to keep getting home location info to keep it updated? do we want to share all home locations and average?
      //probable not, unless GPS are accurate together. Could find average location if rover is unable to find home location
      //for too long of a time, >30 seconds?

      centerOdom.theta = centerLocation.theta;
      cout <<"center x = " << centerOdom.x << " center y = " << centerOdom.y << endl;
  ...
  {
    timeCnt++;
    //main alg in LogicController
    humanTime();//time in h/m/s <-not this exact format though
    ...
    //cout <<"center x = " << updateCenterLocation()->x << "center y = " << updateCenterLocation()->y << endl;

    bool wait = false;

    if (timeCnt < pauseCnt || logicController.pickUpController.lockTarget){
      //run and then pause for an equal amount of ticks
      //used to give the camera enough time to not be blurry
      //also keep running after a cube has been found and trying to pick it up
    ...
    else if (timeCnt < 2*pauseCnt){//pause for an equal amount of time as run
      wait = true;//modify so it doesnt pause only when its doing precision to pick uup a cube
      //cout << "Waiting" << endl;
    }
    else {
      timeCnt = 0;
    ...
    /* Either wait and do nothing if result.type = behaviour && result.b = wait
    * or move motors and gripper for rest of this funct
    * all logic happens inside of logicController
    */
    ...
    //cout << endl;
    //sendDriveCommand(255.0,255.0);//hardcode straight
  }
  ...
  else
  {
    ...
    // poll the logicController to get the waypoints that have been reached.
    std::vector<int> cleared_waypoints = logicController.GetClearedWaypoints();

    for(std::vector<int>::iterator it = cleared_waypoints.begin(); it != cleared_waypoints.end(); it++)
    {
...
void nameListHandler(const std_msgs::String::ConstPtr& names) {
  // obtains swarmie name list, format is #swarmies swarmie0 swarmie1 ... swarmieN
  // use if it is wanted to make the swarmies do different things,
  // ea. alpha swarmie stays by home, rest go and search

  std::size_t namefound = names->data.find(publishedName);

  if(namefound == std::string::npos){//if name is not in list republish it
    //should catch if a previous swarmie goes offline in middle of run
    //will not know how to handle if swarmie running MASTER_NAMELIST node goes offline
    std_msgs::String nameList;
    nameList.data = publishedName;
    namePublisher.publish(nameList);
    broadcastName = 1;
  }
  //if name is in list
  else if(ssnameList.str().compare(names->data) != 0){//check that swarmie has full list
    ssnameList.clear();
    ssnameList.str(names->data);//save namelist
    broadcastName = 0;
  }
  if(broadcastName != 1){
    stringstream temp;
    temp.str("");
    temp << names->data;//can handle as many as you throw at it
    temp >> swarmCount;
    cout << "current number of connected swarmies: " << swarmCount <<endl;

    broadcastName = 1;//only run this block when list changes
  }
}
...
void odometryHandler(const nav_msgs::Odometry::ConstPtr& message) {
  ...
  if(!initialized2){//initialize the initializer HUEHUE, not tested
    initialized = 0;//gets an inital value for the current location before runing intialize in main loop
    initialized2 = 1;
  }
